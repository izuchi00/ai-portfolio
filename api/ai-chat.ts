  ...
  try {
    ...
    for (const m of CANDIDATES) {
      ...
      if (result.ok) { // success
        ...
        return res.status(200).json({ response: answer ?? 'No text generated.', model: m });
      }
    }

    // if we got here, all models failed
    return res.status(502).json({
      error: 'Hugging Face request failed for all candidates',
      tried: CANDIDATES,
      lastStatus: result?.status,
      lastBodySample: result?.text?.slice(0, 200)
    });
